...
function addEventMenu(){
  ...
  if(!Array.isArray(_eventMenusTemp)) _eventMenusTemp=[];
  const newMenu={time:'',title:'',items:''};
  _eventMenusTemp.push(newMenu);
  const container=document.getElementById('ev-food-menus');
  const targetEventId=_eventFoodMenuModalEventId||_editing.event;
  const canViewLikes=!!targetEventId&&Auth.isOrganizer(targetEventId);
  const likeCounts=canViewLikes?getEventFoodLikeCounts(targetEventId):new Map();
  if(container){
    if(container.textContent?.includes('Add one or more menu sections')){
      container.innerHTML='';
    }
    container.insertAdjacentHTML('beforeend', buildEventMenuEditorRow(newMenu,_eventMenusTemp.length-1,likeCounts,canViewLikes));
  }else{
    renderEventMenusEditor();
  }
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('./sw.js?v=20260420-2').catch(() => {});
  });
}
